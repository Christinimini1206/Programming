    ...
    if initial == '2':  # If the initial type is binary
        ...
        for h in range(0, len(number)):
            if ord("A") <= ord(number[h]) <= ord("F"):  # If the number is letter
                number[h] = str(ord(number[h]) - 55)  # Convert it to number
        if initial == "16":
            print(makeOneString(number, 1))
        print("")

        # Change each number to a binary number
        for bi in range(0, len(number)):
            print(f"Convert {number[bi]} to a binary number: ")
            number[bi] = convertDec(number[bi], 2)
            number[bi] = makeOneString(number[bi], 0)
            print(number[bi])
        number = addExtraZero(number, initial)
        if final == "2":  # If the final base is binary
            output = makeOneString(number, 0)
        else:  # If the final base is not binary
            print(f"Converted to Binary: {makeOneString(number, 1)}")
            # Convert the number to the number with final base type
            regrouped = []
            print("")
            print(makeOneString(number, 1))
            if initial == '8' and final == '16':  # Octal to Hex
                regrouped = convertBinGroups(makeOneString(number, 0), final)
            elif initial == '16' and final == '8':  # Hex to Octal
                regrouped = convertBinGroups(makeOneString(number, 0), final)
            elif final == '10':  # Octal/Hex to Decimal
                print(makeOneString(number, 0))
                regrouped = [makeOneString(number, 0)]
            print("")
